...
export function MangaCard({ manga, variant = 'default' }: MangaCardProps) {
  ...
  if (variant === 'compact') {
    return (
      <Link to={`/manga/${manga.id}`} className="group flex gap-3 p-2 hover:bg-muted rounded-lg transition-colors">
        <img src={manga.coverImage} alt={manga.title} className="w-16 h-20 object-cover rounded" />
        <div className="flex-1 min-w-0">
          <h3 className="font-medium text-sm line-clamp-2 group-hover:text-primary transition-colors">{manga.title}</h3>
      ...
      <div className="relative overflow-hidden rounded-lg bg-card">
        <div className="aspect-[3/4] relative">
          ...
          {manga.status === 'ongoing' && (
            <Badge className="absolute top-2 left-2 bg-primary text-primary-foreground">Đang tiến hành</Badge>
          )}
          {manga.status === 'completed' && (
            <Badge className="absolute top-2 left-2 bg-secondary text-secondary-foreground">Hoàn thành</Badge>
          )}
        ...
        <div className="p-3">
          <h3 className="font-semibold line-clamp-2 group-hover:text-primary transition-colors">{manga.title}</h3>
